...
export const SurfaceCodeDiagram = () => {
    ...
    return (
        <div className="flex flex-col items-center p-8 bg-white rounded-xl shadow-sm border border-stone-200 my-8">
            <h3 className="font-serif text-xl mb-4 text-stone-800">Interactive: Distraction Detection</h3>
            <p className="text-sm text-stone-500 mb-6 text-center max-w-md">
                Click the <strong>Nodes</strong> to simulate attention drift. Watch the <strong>Monitors</strong> activate to block the distraction source.
            </p>
            ...
            <div className="mt-6 flex items-center gap-4 text-xs font-mono text-stone-500">
                <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full bg-stone-800"></div> Drift</div>
                <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-sm bg-blue-500"></div> App Block</div>
                <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-sm bg-red-500"></div> Net Block</div>
            </div>

            <div className="mt-4 h-6 text-sm font-serif italic text-stone-600">
                {errors.length === 0 ? "Attention stable." : `Blocking ${activeStabilizers.length} distraction sources.`}
            </div>
        </div>
    );
};
...
export const TransformerDecoderDiagram = () => {
    ...
    return (
        <div className="flex flex-col items-center p-8 bg-[#F5F4F0] rounded-xl border border-stone-200 my-8">
            <h3 className="font-serif text-xl mb-4 text-stone-900">NFC Authentication Loop</h3>
            <p className="text-sm text-stone-600 mb-6 text-center max-w-md">
                The Cookies device uses secure NFC tokens to physically verify your intent to unlock apps, creating a tangible barrier to mindless scrolling.
            </p>
            ...
            <div className="relative w-full max-w-lg h-56 bg-white rounded-lg shadow-inner overflow-hidden mb-6 border border-stone-200 flex items-center justify-center gap-8 p-4">
                ...
                <div className="flex flex-col items-center gap-2">
                    <div className={`w-16 h-16 rounded-lg border-2 flex flex-col items-center justify-center transition-colors duration-500 ${step === 0 ? 'border-nobel-gold bg-nobel-gold/10' : 'border-stone-200 bg-stone-50'}`}>
                        <div className="grid grid-cols-2 gap-2">
                             <div className="w-6 h-6 rounded-full bg-stone-800/80"></div>
                        </div>
                    </div>
                    <span className="text-[10px] uppercase font-bold tracking-wider text-stone-500">PHYSICAL TOKEN</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase font-bold tracking-wider text-stone-500">VERIFICATION</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase font-bold tracking-wider text-stone-500">UNLOCK</span>
                </div>
...
// --- PERFORMANCE CHART ---
export const PerformanceMetricDiagram = () => {
    const [metric, setMetric] = useState('focus');

    // Stats
    const data = {
        focus: { standard: 15, cookies: 45 }, // Minutes of focus time
        pickups: { standard: 80, cookies: 20 }  // Pickups per day
    };

    const currentData = data[metric];
    const maxVal = Math.max(currentData.standard, currentData.cookies) * 1.25;
    ...
    return (
        <div className="flex flex-col md:flex-row gap-8 items-center p-8 bg-stone-900 text-stone-100 rounded-xl my-8 border border-stone-800 shadow-lg">
            <div className="flex-1 min-w-[240px]">
                <h3 className="font-serif text-xl mb-2 text-nobel-gold">Reclaiming Your Time</h3>
                <p className="text-stone-400 text-sm mb-4 leading-relaxed">
                    By introducing a physical cost to unlocking your phone, Cookies drastically reduces unconscious scrolling and increases deep work sessions.
                </p>
                <div className="flex gap-2 mt-6">
                     <button
                        onClick={() => setMetric('focus')}
                        className={`px-3 py-1.5 rounded text-sm font-medium transition-all duration-200 border ${metric === 'focus' ? 'bg-nobel-gold text-stone-900 border-nobel-gold' : 'bg-transparent text-stone-400 border-stone-700 hover:border-stone-500 hover:text-stone-200'}`}
                    >
                        Focus Sessions (min)
                    </button>
                    <button
                        onClick={() => setMetric('pickups')}
                        className={`px-3 py-1.5 rounded text-sm font-medium transition-all duration-200 border ${metric === 'pickups' ? 'bg-nobel-gold text-stone-900 border-nobel-gold' : 'bg-transparent text-stone-400 border-stone-700 hover:border-stone-500 hover:text-stone-200'}`}
                    >
                        Phone Pickups / Day
                    </button>
                </div>
                <div className="mt-6 font-mono text-xs text-stone-500 flex items-center gap-2">
                    ...
                    <span>DATA FROM EARLY BETA USERS</span>
                </div>
            </div>
            ...
            <div className="relative w-64 h-72 bg-stone-800/50 rounded-xl border border-stone-700/50 p-6 flex justify-around items-end">
                ...
                <div className="w-20 flex flex-col justify-end items-center h-full z-10">
                    ...
                    <div className="h-6 flex items-center text-xs font-bold text-stone-500 uppercase tracking-wider">Normal</div>
                </div>

                {/* Cookies Bar */}
                <div className="w-20 flex flex-col justify-end items-center h-full z-10">
                    <div className="flex-1 w-full flex items-end justify-center relative mb-3">
                        <div className="absolute -top-5 w-full text-center text-sm font-mono text-nobel-gold font-bold bg-stone-900/90 py-1 px-2 rounded backdrop-blur-sm border border-nobel-gold/30 shadow-sm">{currentData.cookies}</div>
                        <motion.div
                            className="w-full bg-nobel-gold rounded-t-md shadow-[0_0_20px_rgba(197,160,89,0.25)] relative overflow-hidden"
                            initial={{ height: 0 }}
                            animate={{ height: Math.max(1, (currentData.cookies / maxVal) * 100) + '%' }}
                    ...
                    <div className="h-6 flex items-center text-xs font-bold text-nobel-gold uppercase tracking-wider">With Cookies</div>
                </div>
            </div>
        </div>
    )
